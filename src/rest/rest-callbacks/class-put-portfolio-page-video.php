...
class Put_Portfolio_Page_Video extends Config implements Rest_Callback {
  ...
  public function rest_callback( \WP_REST_Request $request ) {
    ...
    $video_description    = General_Helper::sanitize_html_input( General_Helper::get_array_value( 'videoDescription', $body ) );
    $video_animation_file = General_Helper::sanitize_media( General_Helper::get_array_value( 'videoAnimationFile', $body ) );
    ...
    foreach ( $projects as $project ) {
      ...
      foreach ( $project as $key => $item ) {
        ...
        if ( $key === 'image' ) {
          $sanitized_project[ $key ] = General_Helper::sanitize_media( $item );
          continue;
        }
    ...
    $this->save_options( $video_animation_file, self::VIDEO_ANIMATION_FILE );
