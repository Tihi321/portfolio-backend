      ...
      <Placeholder
        ...
        <form
          ...
          <input
            ...
            placeholder={__('Enter URL to embed here…', 'portfolio-backend')}
            onChange={(e) => {
              onUrlChange(e.currentTarget.value);
            }}
          />
          <Button isLarge type="submit">
            {__('Embed', 'portfolio-backend')}
          </Button>
          {!this.state.validId &&
          <p className="components-placeholder__error">{__('Sorry, we could not embed that content.', 'portfolio-backend')}</p>
          }
        </form>
      </Placeholder>
    );
